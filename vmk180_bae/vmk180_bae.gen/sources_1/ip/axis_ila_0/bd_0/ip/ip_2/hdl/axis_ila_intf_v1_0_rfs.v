...
(* DONT_TOUCH = "TRUE" *)
    ...
    #(
      ...
      parameter  [2:0]  C_NUM_COUNTER          = 3'b001,
     ...
     (
      ...
      output                                  tas_enable_iclk_o,

      input 				      aresetn,
      output				      aresetn_out,
      input                                   trigger_tb_i,
      input [C_RAM_WIDTH-1:0]                 trigger_sample_i,
      input                                   last_flag_i,
      input [64-1:0]                          gap_count_i,
      output reg [10-1:0]                     max_depth_ext_strg_o 
     );
   ...
   localparam [3:0]  IDLE         = 4'b0000,        // This is the initial/idle state
    ...
    localparam TRIGGER_NO_OF_BUSES = RAM_WIDTH_ROUNDED/32 + 3; 
    localparam LC_TRIGGER_INFO_NUM_PACK = TRIGGER_NO_OF_BUSES;
    localparam LC_TRIG_MEM_ADDR_WIDTH   = (clogb2(TRIGGER_NO_OF_BUSES-1)); 
  ...
  begin
    max_depth_flag <= |window_depth_config[63:10];
    if (max_depth_flag)
    begin
     max_depth_ext_strg_o <= 'd1023;
    end 
    else
    begin
      max_depth_ext_strg_o <= ~(~(window_depth_config[9:0]) + 1'b1); 
    end
  end
...
//------ Core config reg inst ----------------------------------------
  ...
  wire tasen_in;
  wire halt_o_iclk;
  (* DONT_TOUCH = "TRUE" *)reg tas_enable;
  (* DONT_TOUCH = "TRUE" *)reg tas_enable_iclk;
  //wire tas_enable;
  reg halt_status_s;
  ...
  begin
    if (halt_o_iclk == 1'b0)
    begin
      tas_enable_iclk <= tas_enable_iclk;
    end
    else
    begin
      tas_enable_iclk <= 1'b0;
    end
  end
  assign tas_enable_iclk_o = tas_enable_iclk;
...
//  assign core_rd_en = (opcode == CORE_CONFIG) ? reg_rd_en_async : 1'b0;
  ...
  // end
  /*(* DONT_TOUCH = "TRUE" *)reg [15:0] count_tas = 'h4; 
  (* DONT_TOUCH = "TRUE" *)wire temp;
  (* DONT_TOUCH = "TRUE" *)wire tas_rst;
  ...
  begin
    ...
    begin
      count_tas <= count_tas - 'h1;
    end 
  end
  
  assign temp = (count_tas == 'h0) ? 1'b1 : 1'b0;
  ...
  assign aresetn_out        = (tas_enable_iclk == 1'b1) ? 1'b1 : ~(halt_o_iclk || ~aresetn);
  ...
  always @(posedge S_AXIS_ACLK)
   begin
     if (s_axis_aresetn_int == 1'b0) begin
        halt_o <= 1'b0;
     end else begin
...
//--------Core detection ---------------------------------------------
  ...
  wire [LC_TRIGGER_INFO_NUM_PACK-1:0] trigger_info_burst_len;
  ...
  reg [LC_TRIG_MEM_ADDR_WIDTH-1:0] trig_mem_addr; 
  ...
  assign status_burst_len = (opcode == CORE_INFO) ? core_detect_burst_len : (opcode == STATIC_INFO) ? static_status_burst_len : (opcode == CORE_CONFIG) ? core_burst_len : (opcode == TRIGGER_INFO_PACKET) ? trigger_info_burst_len : (opcode == PROBE_INFO) ? probe_info_burst_len : dynamic_status_burst_len; 
       ...
       if (!m_axis_aresetn_int)
        ...
        begin
          if (M_AXIS_TREADY == 1'b1 && M_AXIS_TVALID == 1'b1)
           begin
             reg_dynamic_status_arry_0 <= {18'h0, tsm_flag_i[3:0], tsm_state_i[3:0], tas_enable, halt_status_s, cap_state_i[1:0],cap_done_i,arm_s};
...
//-------TRIGGER_INFO_PACKET------------------------------------------------------
 ...
 begin
   arm_t_dly  <= arm_s;
   arm_t_dly2 <= arm_t_dly;
 end
 assign arm_posedge = arm_t_dly & ~arm_t_dly2;

 
 reg                          reg_trigger_bit = 1'b0;
 reg [RAM_WIDTH_ROUNDED-1:0] reg_trigger_sample_i ;
 reg [RAM_WIDTH_ROUNDED +(C_DATA_WIDTH *3) -1:0] reg_trigger_info_complete ;
 reg [C_DATA_WIDTH-1:0] trigger_info_w0 ;
 wire [C_DATA_WIDTH-1:0] DOUT_O;
 reg [C_DATA_WIDTH-1:0] reg_gap_count_l       = {C_DATA_WIDTH{1'b0}};
 reg [C_DATA_WIDTH-1:0] reg_gap_count_u       = {C_DATA_WIDTH{1'b0}};
 reg 	  	        reg_last_flag         = 1'b0;

 wire [C_DATA_WIDTH-1:0] trigger_info_rd_data_o;
 wire trigger_info_rd_en ; 
 assign trigger_info_rd_en = (opcode == TRIGGER_INFO_PACKET) ? rd_en_temp : 1'b0;
 always @ (posedge M_AXIS_ACLK)
      begin 
        if (!m_axis_aresetn_int | arm_posedge)
         begin
          reg_trigger_bit       <= 1'b0;
          reg_trigger_sample_i  <= {RAM_WIDTH_ROUNDED{1'b0}};
	  reg_gap_count_l       <= {C_DATA_WIDTH{1'b0}};
	  reg_gap_count_u       <= {C_DATA_WIDTH{1'b0}};
	  trigger_info_w0       <= {C_DATA_WIDTH{1'b0}};
	  reg_last_flag         <= {1'b0};
          reg_trigger_info_complete <= {RAM_WIDTH_ROUNDED +(C_DATA_WIDTH *3){1'b0}};
         end
        else
         begin
           reg_gap_count_l       <= gap_count_i[32-1:0];
	   reg_gap_count_u       <= gap_count_i[64-1:32];
	   reg_last_flag         <= last_flag_i;
           reg_trigger_sample_i  <= trigger_sample_i;
           trigger_info_w0       <= {reg_last_flag,reg_trigger_bit};
           reg_trigger_info_complete <= {reg_gap_count_u,reg_gap_count_l,reg_trigger_sample_i,trigger_info_w0}; 
           if (trigger_tb_i == 1'b1)
           begin
              reg_trigger_bit    <= 1'b1 ;               
           end
           else 
           begin
              reg_trigger_bit    <= reg_trigger_bit;
           end       
         end
       end
  axis_ila_intf_v1_0_0_generic_mux
  #(
    .C_WIDTH(C_DATA_WIDTH),
    .C_NUM_BUSES(TRIGGER_NO_OF_BUSES)
  ) trigger_data_mux (
    .DIN_I(reg_trigger_info_complete),
    .SEL_I(trig_mem_addr),
    .DOUT_O(DOUT_O)
  ); 

 assign trigger_info_rd_data_o = DOUT_O;
 assign trigger_info_burst_len = (rd_wr == LC_COMPLETE_READ) ? LC_TRIGGER_INFO_NUM_PACK : rdwr_burst_len[clogb2(LC_TRIGGER_INFO_NUM_PACK)-1:0];                                     
//------ STATIC_INFO_PACKET---------------------------------------------------------
  ...
  localparam [4:0] LC_COUNTER0_WIDTH = (C_NUM_COUNTER >= 3'b001 && C_HAS_ADV_TRIG == 1) ? C_COUNTER_WIDTH : 5'b00000;
  localparam [4:0] LC_COUNTER1_WIDTH = (C_NUM_COUNTER >= 3'b010 && C_HAS_ADV_TRIG == 1) ? C_COUNTER_WIDTH : 5'b00000;
  localparam [4:0] LC_COUNTER2_WIDTH = (C_NUM_COUNTER >= 3'b011 && C_HAS_ADV_TRIG == 1) ? C_COUNTER_WIDTH : 5'b00000;
  localparam [4:0] LC_COUNTER3_WIDTH = (C_NUM_COUNTER >= 3'b100 && C_HAS_ADV_TRIG == 1) ? C_COUNTER_WIDTH : 5'b00000;
  localparam [2:0] LC_NUM_COUNTER    = (C_HAS_ADV_TRIG == 1) ? C_NUM_COUNTER : 3'b000;
  ...
  assign static_status_burst_len = (rd_wr == LC_COMPLETE_READ) ? LC_STATIC_INFO_NUM_PACK : rdwr_burst_len[LC_STATIC_INFO_NUM_PACK-1:0];
   ...
   assign reg_static_info_arry[1] = {{(C_DATA_WIDTH-9){1'b0}}, C_HAS_DEEP_STORAGE, C_CC_TYPE, LC_NUM_COUNTER[2:0], C_HAS_TRIG_OUT, C_HAS_TRIG_IN, C_HAS_STRG_QUAL, C_HAS_ADV_TRIG};
...
axis_ila_intf_v1_0_0_axis_valid_last_gen #(
 ...
 (
  ...
  assign axis_tdata_reg = (opcode == CORE_INFO) ? core_detect_rd_data_o : (opcode == STATIC_INFO) ? core_static_info_rd_data_o : (opcode == PROBE_INFO) ? probe_info_rd_data_o : (opcode == DYNAMIC_STATUS) ? dynamic_status_rd_data_o : (opcode == CORE_CONFIG) ? core_rd_data_o : (opcode == TRIGGER_INFO_PACKET) ? trigger_info_rd_data_o : (opcode == MU_CONFIG_1K) ? mu_rd_data_o : (opcode == TRACE_DATA) ? data_word_out_i : {C_DATA_WIDTH{1'b0}};
...
/*    trace_data_addr_gen_tx_data #(
    ...
    ( 
       ...
       .m_axis_aresetn(m_axis_aresetn_int),
  ...
  (* DONT_TOUCH = "TRUE" *)reg axis_tlast_temp   = 1'b0;

  always @(posedge M_AXIS_ACLK)
   begin
     if (m_axis_aresetn_int == 1'b0)
      begin
        ...
        mem_addr               <= rdwr_addr_sig[LC_CORE_DECT_NUM_PACK-1:0];
        trig_mem_addr          <= rdwr_addr_sig[LC_TRIG_MEM_ADDR_WIDTH-1:0];
        status_cnt_burst_len   <= {LC_CORE_DECT_NUM_PACK{1'b0}};                     
      ...
      begin  
		...
		if (M_AXIS_TREADY == 1'b1 && M_AXIS_TVALID == 1'b1 && M_AXIS_TLAST == 1'b1) // && M_AXIS_TVALID == 1'b1)
                 begin
                   mem_addr <= {LC_CORE_DECT_NUM_PACK{1'b0}};
                   trig_mem_addr  <=  {LC_TRIG_MEM_ADDR_WIDTH{1'b0}};
                 end
...
module axis_ila_intf_v1_0_0_reg_array #(
  ...
  (
   ...
   (* DONT_TOUCH = "TRUE" *)output reg done_o,
 ...
 *----------------------------------------------------------------------------*/
`timescale 1 ns / 1 ps
module axis_ila_intf_v1_0_0_generic_mux
  #(
